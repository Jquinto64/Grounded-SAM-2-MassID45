...
from sahi.models.geminisam2 import GeminiSam2DetectionModel
from sahi.predict import get_prediction, get_sliced_prediction, predict
...
def load_gemini_api_key(file_path='config.yaml'):
    """
    ...
    Raises:
        ...
        KeyError: If the API key is not found in the expected structure.
...
def main(args):
    ...
    # Create the model
    detection_model = GeminiSam2DetectionModel(
        sam2_checkpoint=args.sam2_checkpoint,
        sam2_config=args.sam2_config,
        model_name=args.model_name,
        api_key=load_gemini_api_key("gemini_secrets.yaml"),
    ...
    follow_rate_limit = args.model_name == "gemini-2.5-pro-exp-03-25"
    if args.predict:
        predict(
            ...
            name = exp_name,
            follow_rate_limit=follow_rate_limit,
        )
    ...
    # Edge case: Handling empty polygon or ill-defined polygon (<3 points) within a mask
    for pred in pred_results:
...
if __name__ == "__main__":
    """
    model = GeminiSam2DetectionModel(
        ...
        confidence_threshold=0.25, # arbitrary as we don't actually return confidences from this model
        device=device
    )
    """
    ...
    parser.add_argument("--model_name", type=str, default="gemini-2.0-flash", help="Name of Gemini model; should be one of ['gemini-1.5-pro-002','gemini-1.5-flash-latest','gemini-2.0-flash-lite','gemini-2.0-flash-thinking-exp-01-21','gemini-2.0-flash','gemini-2.5-pro-exp-03-25']")
    parser.add_argument("--text_prompt", type=str, default="Return bounding boxes around small brown insects, ants, flies, and gnats as JSON arrays [ymin, xmin, ymax, xmax]. Exclude loose insect wings and loose insect parts.", help="Text prompt for Gemini to detect insects")
    parser.add_argument("--temperature", type=float, default=0.5, help="Parameter for setting the 'creativeness' of the LLM; recommended value for object detection is 0.5")
